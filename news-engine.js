document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Run Header Plugins (Time/Weather) - Does not affect news boxes
    ...
    // --- ZONE 3: DATA FETCH & GENERAL NEWS FILTER ---
    fetch(jsonUrl).then(res => res.json()).then(data => {
        ...
        const filteredData = data.filter(item => {
            ...
            const isCountyWide = item.tags.includes("Clay County");
    ...
    function renderSummary(item) {
        ...
        // BREAKOUT LOCK: target="_top" forces link to open the main Hub site
        summaryContainer.innerHTML += `
    ...
    function handleScroll() {
        ...
        if (targetId) {
            ...
            if (element) {
                element.scrollIntoView({ behavior: 'smooth', block: 'start' }); // Drops you to the article
            }
        }
    }
