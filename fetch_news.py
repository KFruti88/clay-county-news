...
# ADDED: Blacklist to block specific towns/keywords
BLACKLIST = ["Cisne"]
...
def get_metadata(text):
    ...
    # NEW: BLACKLIST CHECK
    # If "Cisne" is found anywhere, we return None to tell the script to SKIP this story.
            ...
            if resp.status_code == 200:
                ...
                for art in soup.find_all('article')[:3]:
                    ...
                    if title_node:
                        ...
                        # Only proceed if story was NOT blacklisted (cat is not None)
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for item in root.findall("./channel/item")[:35]:
                    ...
                    # SKIP if blacklisted (cat will be None)
                    if cat is None:
                        continue
    ...
    for result_set in results:
        for s in result_set:
            ...
            if h not in seen_hashes:
                final_news.append({
                    "id": h, "title": s['title'], "description": s['description'],
                    "category": s['category'], "tags": s['tags'], "link": f"{NEWS_CENTER_URL}#{h}", "date_added": timestamp
                })
                seen_hashes.add(h)
