...
def get_metadata(text):
    ...
    # 1. STRICT BLACKLIST (Stops Cisne news immediately)
    if any(forbidden.lower() in text_lower for forbidden in BLACKLIST):
        return None, None

    cat = "General News"
    
    # 2. PRIORITY CATEGORY: Arts (Ensures Music/ReVue isn't called Police)
    ...
    # If state news or multiple towns mentioned, tag as County News
