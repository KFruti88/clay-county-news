...
def weld_text(text):
    """Glues dates and decimals together for clean layout."""
    if not text: return ""
...
def clean_text(text):
    """Removes signatures but preserves emails."""
    if not text: return ""
    text = re.sub(r'<[^>]+>', ' ', text)
    patterns = [
...
async def get_full_content(url):
    """Scrapes the full article body from the web."""
    try:
...
def get_metadata(text):
    """Tags every town mentioned. If none, tags 'County News'."""
...
async def process_news():
    final_news = []
    seen_ids = set() # STRICT DUPLICATE TRACKER
    ...
    print(f"Update complete. {len(final_news)} unique stories saved.")
