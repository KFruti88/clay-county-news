...
def get_metadata(text):
    ...
    # 1. STRICT BLACKLIST (Must be first)
    if any(forbidden.lower() in text_lower for forbidden in BLACKLIST):
        return None, None

    # 2. PRIORITY CATEGORY: Arts (Prevents ReVue being called Police)
    if any(word in text_lower for word in ['musical', 'music', 'concert', 'band', 'revue', 'burmashavers']):
        ...
        cat = "State News"
    elif any(word in text_lower for word in ['arrest', 'sheriff', 'police', 'blotter', 'jail', 'booked']):
        cat = "Police Report"
                ...
                for art in soup.find_all('article')[:3]:
                    ...
                    if title_node:
                        ...
                        if cat and (tags != ["County News"] or cat != "General News"):
                            scraped_stories.append({"title": clean_text(t_text), "description": clean_text(b_text), "category": cat, "tags": tags})
        except: pass
            ...
            if resp.status_code == 200:
                ...
                for item in root.findall("./channel/item")[:35]:
                    ...
                    if content_hash not in seen_hashes:
                        ...
                        final_news.append({
                            "id": story_id, "title": clean_title, "description": clean_text(desc),
                            "category": cat, "tags": tags, "link": f"{NEWS_CENTER_URL}#{story_id}", "date_added": timestamp
                        })
    ...
    for result_set in results:
        for s in result_set:
            ...
            if h not in seen_hashes:
                final_news.append({"id": h, "title": s['title'], "description": s['description'], "category": s['category'], "tags": s['tags'], "link": f"{NEWS_CENTER_URL}#{h}", "date_added": timestamp})
                seen_hashes.add(h)
