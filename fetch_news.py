...
# This is the ONLY source link for the news
SOURCE_FEED = "https://www.wnoi.com/category/local/feed"
...
def get_metadata(text):
    """Identifies mentioned towns for tagging. Fallback is 'County News'."""
...
async def process_news():
    final_news = []
    seen_ids = set() # PREVENTS DUPLICATES
    timestamp = datetime.now().isoformat()

    async with httpx.AsyncClient() as client:
        try:
            resp = await client.get(SOURCE_FEED, timeout=15)
            if resp.status_code == 200:
    ...
    print(f"News Updated: {len(final_news)} unique stories processed.")
